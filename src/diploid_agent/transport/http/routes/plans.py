...
def register_plans(
    ...
    @app.get(
        "/plan/list", response_model=list[PlanResponse], dependencies=[Depends(_require_api_key)]
    )
    ...
    @app.get(
        "/plan/{plan_id}", response_model=PlanResponse, dependencies=[Depends(_require_api_key)]
    )

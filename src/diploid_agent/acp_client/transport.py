...
from diploid_agent.acp_client.callbacks import AcpCallbackPump
...
class AcpTransport:
    ...
    def __init__(
        ...
        # block on locks or I/O; the pump serializes them off the reader
        # thread on a bounded queue so a wedged callback can never
        # backpressure the reader (overflow drops work instead).
        self._cb_pump = AcpCallbackPump(self._state)
    ...
    async def close(self) -> None:
        ...
        # Stop the prompt-callback worker after queued callbacks drain; the
        # sentinel handling lives in the pump, whose join timeout bounds the
        # wait even if the worker is wedged.
    ...
    # ------------------------------------------------- callback pump seams
    # The update -> callback pipeline lives on ``self._cb_pump``
    # (``AcpCallbackPump``).  The ``_cb_*``/``_route_update``/``_dispatch_cb``
    # names below are thin delegates kept so tests and ``AcpClient`` can keep
    # reaching them via ``transport._x``.

    @property
    def _cb_queue(self) -> queue.Queue[tuple[Callable[[Any], None], Any] | None]:
        return self._cb_pump._cb_queue

    @_cb_queue.setter
    def _cb_queue(self, value: queue.Queue[tuple[Callable[[Any], None], Any] | None]) -> None:
        self._cb_pump._cb_queue = value

    @property
    def _cb_thread(self) -> threading.Thread | None:
        return self._cb_pump._cb_thread

    @_cb_thread.setter
    def _cb_thread(self, value: threading.Thread | None) -> None:
        self._cb_pump._cb_thread = value

    @property
    def _cb_dropped(self) -> int:
        return self._cb_pump._cb_dropped

    @_cb_dropped.setter
    def _cb_dropped(self, value: int) -> None:
        self._cb_pump._cb_dropped = value
    ...
    def _dispatch_cb(self, cb: Callable[[Any], None], arg: Any) -> None:
        ...
        self._cb_pump._dispatch_cb(cb, arg)

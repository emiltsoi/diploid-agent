...
from devin_fleet_harness.notifier import NoopNotifier, TelegramNotifier, WebhookNotifier
...
def test_telegram_notifier_retries_read_timeout(monkeypatch) -> None:
    ...
    monkeypatch.setattr("devin_fleet_harness.notifier.time.sleep", lambda *_: None)
...
def test_telegram_notifier_default_client_uses_30s_timeout(monkeypatch) -> None:
    ...
    client.post.return_value.raise_for_status = MagicMock()
    monkeypatch.setattr("devin_fleet_harness.notifier.httpx.Client", lambda **kw: client)

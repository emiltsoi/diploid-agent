#!/usr/bin/env bash
# devin-fleet-harness-run.sh — run BOTH harness processes under one systemd unit.
#
...
# whole pair (Restart=on-failure in devin-fleet-harness.service).
set -euo pipefail
...
.venv/bin/python -m devin_fleet_harness.telegram_poll \
  --config config/harness.yaml &
POLLER_PID=$!

# Start the FastAPI ingress in the background.
.venv/bin/python -m devin_fleet_harness.telegram_ingress \
  --config config/harness.yaml &
INGRESS_PID=$!
...
# that subprocess's exit code so systemd restarts the pair on failure.
set +e
wait -n
EXIT_CODE=$?
set -e

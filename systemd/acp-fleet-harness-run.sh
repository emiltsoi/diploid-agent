#!/usr/bin/env bash
# acp-fleet-harness-run.sh — run BOTH harness processes under one systemd unit.
#
...
# whole pair (Restart=on-failure in acp-fleet-harness.service).
set -euo pipefail
...
.venv/bin/python -m acp_fleet_harness.telegram_poll \
  --config config/harness.yaml &
POLLER_PID=$!

# Start the FastAPI ingress in the background.
.venv/bin/python -m acp_fleet_harness.telegram_ingress \
  --config config/harness.yaml &
INGRESS_PID=$!

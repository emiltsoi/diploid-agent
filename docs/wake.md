...
A separate `diploid-waker` process polls the queue and calls `POST /wake`.
...
Copy `probes/diploid-waker.py` to a convenient location (for example
`~/.local/bin/diploid-waker`) and run it:

```bash
diploid-waker --wake-queue wake_queue.jsonl --harness-url http://127.0.0.1:4003
```

A `systemd/diploid-waker.service.example` unit is included for running it as
a user service.

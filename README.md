...
- Gates agent-initiated restarts on a policy check inside `RuntimeRestart` — never in the sandbox shim: the authorship `restart_enabled` toggle must be on, a non-empty `reason` is required (incident-recorded, operator-notified), and the target unit must be in `harness.restart_allowed_units` (empty = the persona's own unit). `harness_restart` is the discoverable MCP door; the control socket additionally requires a per-boot `DIPLOID_CONTROL_TOKEN`, so same-uid siblings and task-spawned children (whose env advertises an unbound socket) cannot reach a peer's listener. Operator doors (`/graceful_restart`, `POST /graceful-restart`) bypass the gate.
...
- Runs declarative cron jobs from `config/crons.yaml` (operator-owned) and `<persona>/crons.yaml` (agent-authored, gated by the authorship `cron_enabled` toggle): `script` jobs run as shell subprocesses and `llm` jobs run as *phantoms* — fresh isolated ACP children composed with the persona's identity files + promoted pocket and an empty MCP list — materialized through the TaskEngine. Jobs are driven by `schedule:` (interval/cron/daily) or `trigger:` (`file` mtime watches confined to persona/session roots, `body` edge-fires on `chat_body_state.json` compares) with a cooldown floor that gates edge consumption. `delivery: silent|digest|turn` writes per-job result files, renders a bounded `cron` prompt slot, or enqueues a wake that shares the self-wake budgets. `GET /cron` exposes merged job state, `POST /cron/<id>/run` is the operator's manual-fire door, and `harness_cron_list` gives the agent a read-only view. See `docs/cron.md`.
...
- Shows what a turn is actually running during think/tool phases: `last_side_effect` rides the `/turn` status wire and renders as a `· exec: pytest` line in the placeholder while no reply text is streaming (`harness.telegram.tool_progress`, default on).
- Streams wake-driven turns live in Telegram: a `turn_started` outbox marker hands each non-silent wake turn to a `WakeDisplayWorker`, which posts `...` eagerly at T+0, streams the partial reply into it, and finalizes once — with typing presence for the whole turn and a 45-minute hard cap. Interim items (heartbeats, mesh floats, notices) carry `transient` so they deliver standalone instead of killing the stream. `harness.telegram.wake_stream` (default on) gates the display; silent wakes stay typing-only.
- Pushes the full slash-command surface into Telegram's native "/" menu via `setMyCommands` at poller start — overwritten each boot so it self-heals with the code; `harness.telegram.bot_menu` (default on) gates it.
...
- `/graceful_restart [service]` — schedule a graceful systemd restart of the named service (default: the current persona's `.service` unit).
